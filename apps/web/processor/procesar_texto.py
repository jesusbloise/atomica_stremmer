...
# ----------------- Main -----------------
def main(upload_id: str, signed_url: Optional[str] = None):
    conn = None
    try:
        ...
        source = signed_url if signed_url else file_path
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    signed_url = sys.argv[2] if len(sys.argv) >= 3 else None
    main(sys.argv[1], signed_url)
